...
namespace ExampleApp.Identity
{
    public class Full2FARequiredFilterAttribute : Attribute, IAsyncPageFilter, IAsyncActionFilter
    {
        ...
        public async Task<IActionResult> ApplyPolicy(HttpContext context)
        {
            ...
            if (!(await authService.AuthorizeAsync(context.User, "Full2FARequired")).Succeeded)
            {
                return new RedirectToPageResult("/Full2FARequired", new { returnUrl = Path(context) });
            }
            return null;
        }
        ...
        private string Path(HttpContext context)  => $"{context.Request.Path}{context.Request.QueryString}";
    }
}
