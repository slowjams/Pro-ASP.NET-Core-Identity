    ...
    public class UserRolesModel : PageModel
    {
        ...
        public async void OnGet()
        {
            ...
            if (user != null)
            {
                ...
                Roles = (await UserManager.GetClaimsAsync(user))?.Where(c => c.Type == ClaimTypes.Role).Select(c => c.Value);

                AvailableRoles = new SelectList(RoleManager.Roles.OrderBy(r => r.Name).Select(r => r.Name).Except(Roles));
            }
        }
        ...
        public async Task<IActionResult> OnPostAdd(string newRole)
        {
            ...
            await UserManager.AddClaimAsync(await GetUser(), new Claim(ClaimTypes.Role, newRole));
        ...
        private Task<AppUser> GetUser() => Id == null ? null : UserManager.FindByIdAsync(Id);
    }
}
