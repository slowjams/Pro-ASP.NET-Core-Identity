    ...
    public class EmailPhoneChangeModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPost(string id, string dataValue)  // dataValue is the new email address or phone number
        {
            ...
            if (IsEmail)
            {
                ...
                EmailSender.SendMessage(AppUser, "Confirm Email", "Please click the link to confirm your email address:", $"http://localhost:5000/validate/{id}/email/{dataValue}:{token}");
            }
            else
            {
