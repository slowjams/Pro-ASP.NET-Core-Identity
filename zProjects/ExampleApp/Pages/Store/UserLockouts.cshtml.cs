...
namespace ExampleApp.Pages.Store
{
    public class UserLockoutsModel : PageModel
    {
        ...
        public IEnumerable<AppUser> Users => UserManager.Users
            // UserManager.IsLockedOutAsync() internally calls IUserLockoutStore.GetLockoutEndDateAsync() and compare it with DateTimeOffset.UtcNow
        ...
        public async Task<string> GetLockoutStatus(AppUser user)
        {
            if (await UserManager.IsLockedOutAsync(user))
            {
                ...
                return $"Locked Out ({remaining.Minutes} mins " + $"{remaining.Seconds} secs remaining)";
            }
        ...
        {
            await UserManager.SetLockoutEndDateAsync(await UserManager.FindByIdAsync(id), DateTimeOffset.Now.AddMinutes(mins));
